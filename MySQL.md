...
1. 二叉树因为每个节点最多只能容纳两个子节点，随着数据量增大，高度越来越高，导致磁盘IO频繁，高耗能，而且极端情况下可能会退化成链表结构；
2. AVL树是为了解决二叉树极端不平衡的情况，但是高层级导致磁盘IO频繁的问题并没有解决；
3. B-Tree每个节点可以有多个子节点，但是非叶子节点也可以存放数据，这将导致非叶子结点存放的指针数量变少，树层数增高，导致IO操作变多，查询性能变低；
4. B+Tree只有叶子节点才存储数据，一般层数能控制在1-3层，大大减少IO操作，到达快速查询的目的；

##### InnoDB一棵B+树可以存放多少行数据？
这个问题的简单回答是：约2千万
1.在计算机中磁盘存储数据最小单元是 扇区，一个扇区的大小是 512 byte；  
2.文件系统（例如XFS/EXT4）最小单元是 块，一个块的大小是 4 KB（=8个扇区），文件系统中一个文件大小只有1个字节，但不得不占磁盘上4KB的空间；  
3.InnoDB存储引擎也有自己的最小储存单元——页（Page），一个页的大小是16KB（=4个块 =32个扇区），Innodb的所有数据文件（后缀为.ibd的文件），他的大小始终都是16384 byte（16k）的整数倍。  
分析： 
      总数据量 = 叶子节点数 * 单页存放数据条数
      假设主键ID为bigint类型，长度为8字节，而指针大小在InnoDB源码中设置为6字节，这样一共14字节，我们一个页中能存放多少这样的单元，
      其实就代表有多少指针，即16384/14=1170 个单元（单个数据页存储的单元数）；
      通常满树是3层，那么 叶子节点数 = 1170 * 1170； 
      假设生产环境上一条数据1K左右，那么 单页存放数据条数 = 单页大小（默认16K）/ 1K = 16 条；
      所以  总数据量 = 叶子节点数 * 单页存放数据条数 = 1170 * 1170 * 16 = 21902400 约等于 2千万条
